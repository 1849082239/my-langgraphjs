  ...
  if (messages.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full px-4 py-8">
        ...
        <h2 className="text-lg font-medium text-foreground mb-1 text-center">Hello! How can I help you today?</h2>
        ...
        <div className="grid grid-cols-2 gap-2 w-full max-w-md">
          {suggestions.map((suggestion, i) => (
            <button
              ...
              className={`px-4 py-3 rounded-xl bg-[#faf6ef]/88 border border-border text-sm text-left hover:border-primary/30 transition-all card-hover shadow-sm stagger-${
                i + 1
              }`}
            >
              {suggestion.text}
      ...
      {messages.map((msg, i) => (
        <div key={i} className={`flex gap-3 message-enter ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
          ...
          <div className={`flex flex-col max-w-[86%] ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
            <span className="text-xs text-muted-foreground mb-1 px-1">{msg.role === 'user' ? 'You' : 'Aether'}</span>
